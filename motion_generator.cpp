...
#define TOTAL_VEL 0.02 // !DYX! : Ensure consistency with the setting in the master node

// Cartesian trajectory requirements by FCI
#define TRANSLATION_VEL_LIMIT 1.7     // dp.Translation - (m/s)
#define TRANSLATION_ACC_LIMIT 1.30    // ddp.Translation - (m/s^2) : factor 10
#define TRANSLATION_JERK_LIMIT 650.00 // dddp.Translation - (m/s^3) : factor 10
#define ROTATION_VEL_LIMIT 2.5        // dp.Rotation - (rad/s)
#define ROTATION_ACC_LIMIT 2.50       // ddp.Rotation - (rad/s^2) : factor 10
#define ROTATION_JERK_LIMIT 1250.00   // dddp.Rotation - (rad/s^3) : factor 10

struct ControlCommands
{
  double x_velocity = 0.02;
  double y_velocity = 0.02;
  double rotation = M_PI; // Target rotarz position on z-axis
  std::mutex mtx;
}; 
...
int main(int argc, char **argv)
{
  ...
  try
  {
    ...
    robot.control([=, &time](const franka::RobotState &robot_state, franka::Duration period) -> franka::CartesianVelocities
      ...
      ROS_INFO("At time : %f; Robot motion states : %f, %f, %f.", time, robot_state.O_dP_EE_c[0], robot_state.O_dP_EE_c[1], robot_state.O_dP_EE_c[5]);
      ...
      ROS_INFO("Control commands input : %f, %f, %f.", v_x, v_y, w_z);
