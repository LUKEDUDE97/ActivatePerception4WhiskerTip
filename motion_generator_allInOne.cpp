...
#define TOTAL_VEL 0.02 // Ensure consistency
...
struct ControlCommands
{
    ...
    double rotation = 0.0; // Target rotation position on z-axis
    std::mutex mtx;
};
...
int main(int argc, char **argv)
{
    ...
    ros::Publisher robot_state_pub = nh.advertise<geometry_msgs::TwistStamped>("/Robot_State/O_T_EE", 10);
    ...
    try
    {
        ...
        double time = 0.0;
        robot.control([=, &time, &robot_state_pub](const franka::RobotState &robot_state, franka::Duration period) -> franka::CartesianVelocities
                      {
    time += period.toSec();
